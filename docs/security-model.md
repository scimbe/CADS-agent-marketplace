...
| F.14 | `InstallerKind::Binary` has no static-analysis equivalent to `guardrails::scan_compose` -- prior to CADS-agent-marketplace#12, a trusted-but-malicious or compromised publisher's Binary executable ran with FULL, unconfined access to the whole host (filesystem, network, every other process), not just the install -- and no pre-execution warning said so | `installer_engine::sandbox` wraps the executable in a per-OS lightweight sandbox when one is available -- `bubblewrap` on Linux (`installer_engine::sandbox::bwrap`), confining network (no namespace at all -- F.1-equivalent), PID/UTS/IPC namespace sharing (F.2-equivalent), and filesystem writes outside `work_dir` (F.3-equivalent), verified by a real sandboxed-exec probe, not just a `--version` check (some hardened kernels/LSM configs let `--version` succeed while a real sandboxed exec fails). When no backend is usable on a given host (or on macOS/Windows, which have no bwrap-equivalent implemented yet -- see `docs/design/sandbox-fallback.md`'s milestones), activation proceeds unsandboxed by DEFAULT, but a loud `eprintln!` warning fires before every unsandboxed Binary execution either way, naming the publisher and the exact remediation. An operator who wants fail-closed instead sets `CT_REQUIRE_BINARY_SANDBOX=1`, which refuses the activation outright rather than running it unconfined. Documented residual: even sandboxed, there is no manifest-declared policy (every Binary gets one fixed, maximally-restrictive profile) and no resource limits (memory/CPU), matching Compose's own current gap rather than regressing from it. | `installer_engine::sandbox`, `activate::activate` step 9's Binary arm |
